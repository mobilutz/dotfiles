#!/bin/bash
# Fixes a corrupted 192.0.2.0/23 subnet route injected by Docker that causes
# local network devices to become unreachable.
# Logs to ~/Library/Logs/fix-local-route.log for diagnostics.

if netstat -rn -f inet | grep -q "^192.0.2\/23"; then
  ...
  sudo /sbin/route delete 192.0.2.0/23 2>/dev/null
